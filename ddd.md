...
1. Слой персистентности оказывает влияние на модели предметной области.
2. Отсутствие взаимодействия с экспертами в предметной области.
3. Игнорирование общего языка (он должен использоваться не только в обсуждениях, но и в коде).
4. Ограниченные контексты не выделены.
5. Анемичные модели.
6. Отсутствие гибкости. Модель предметной область развивается, следует это учитывать и развивать код вслед за этими изменениями.
7. Предположение о том, что вся логика является доменной логикой.
8. Чрезмерное внимание к тестированию деталей взаимодействия. На самом деле важнее проверять конечное состояние, нежели проверять детали выполнения процесса (которые могут измениться).
9. Считать, что безопасность (авторизация) является частью предметной области. Это второстепенная логика (если только это не является вашей предметной областью).
10. Большое внимание к инфраструктуре.
